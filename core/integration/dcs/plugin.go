...
func (p *Plugin) CallStack(data interface{}) (stack map[string]interface{}) {
	...
	stack["PrepareForRun"] = func() (out string) { // must formally return string even when we return nothing
		var err error
		log := log.WithField("partition", envId).WithField("call", "PrepareForRun")
		...
		err, payloadJson = PFRgRPCCommunication(ctx, err, timeout, call, envId, payloadJson, stream, detectorStatusMap, callFailedStr, payload)
	...
	stack["StartOfRun"] = func() (out string) { // must formally return string even when we return nothing
		...
		log := log.WithField("partition", envId).WithField("call", "StartOfRun")
		...
		log = log.WithField("run", runNumber64)
		...
		err, payloadJson = SORgRPCCommunication(ctx, err, timeout, call, envId, payloadJson, stream, detectorStatusMap, callFailedStr, payload)
	...
	eorFunc := func(runNumber64 int64) (out string) { // must formally return string even when we return nothing
		...
		log := log.WithField("partition", envId).WithField("call", "EndOfRun").WithField("run", runNumber64)
		...
		err, payloadJson = EORgRPCCommunication(ctx, err, timeout, call, envId, payloadJson, stream, detectorStatusMap, callFailedStr, payload)
...
func EORgRPCCommunication(ctx context.Context, err error, timeout time.Duration, call *callable.Call, envId string,
	payloadJson []byte, stream dcspb.Configurator_EndOfRunClient, detectorStatusMap map[dcspb.Detector]dcspb.DetectorState,
	...
	return err, payloadJson
}

func SORgRPCCommunication(ctx context.Context, err error, timeout time.Duration, call *callable.Call, envId string,
	payloadJson []byte, stream dcspb.Configurator_StartOfRunClient, detectorStatusMap map[dcspb.Detector]dcspb.DetectorState,
	callFailedStr string, payload map[string]interface{},
) (error, []byte) {
...
func PFRgRPCCommunication(ctx context.Context, err error, timeout time.Duration, call *callable.Call, envId string,
	payloadJson []byte, stream dcspb.Configurator_StartOfRunClient, detectorStatusMap map[dcspb.Detector]dcspb.DetectorState,
	callFailedStr string, payload map[string]interface{},
) (error, []byte) {
